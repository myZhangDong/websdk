...
module.exports = PCStats = _util.prototypeExtend({
    ...
    _gather_inbound_rtp_packetsLost: function (dataArray) {
        var data = dataArray.shift();
        return data && data.packetsLost || 0;
    },
    _gather_inbound_rtp_packetsReceived: function (dataArray) {
        var data = dataArray.shift();
    ...
    _util.forEach(["Audio", "Video"], function (_trackTypeIndex, trackType) {
        (function gather(gatherType, trackType) {
            ...
            PCStats[gatherType + trackType] = gatherTrack = function (easemobStream, onNotify, intervalMillis) {
                ...
                var intervalId = gatherTrack.__intervalIds && gatherTrack.__intervalIds[easemobStream.id];
                ...
                intervalId = setInterval(function () {
                    ...
                    if(!pcstats
                        ...
                        && (!easemobStream
                            || !easemobStream._webrtc
                            || easemobStream._webrtc.closed
                            || easemobStream._webrtc.__id !== pcstats._webrtc.__id
                            || !easemobStream.getMediaStream()
                    ...
                    if(!pcstats){
                        var cacheStream;
                        if(easemobStream
                            && emedia.__easemob_current_mservice
                            && (cacheStream = emedia.__easemob_current_mservice.getStreamById(easemobStream.id))){
                ...
                gatherTrack.__intervalIds[easemobStream.id] = intervalId;
